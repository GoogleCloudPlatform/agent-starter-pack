...
All agent templates generated by the Agent Starter Pack are automatically instrumented with OpenTelemetry. This provides two levels of observability:

1. **Agent Telemetry Events** (Enabled by Default): OpenTelemetry traces and spans for agent operations, exported to **Cloud Trace**. This tracks agent execution, latency, and system metrics.
...
### Agent Telemetry Events (Enabled by Default)
...
*   **Enabled by Default:** This telemetry is active by default in all environments, including local development (`make playground`) and all Terraform-deployed environments (dev, staging, prod).
