...
6.  **Built-in Tools**: `google_search`, `BuiltInCodeExecutor`, `VertexAiSearchTool`.
...
async def main():
    ...
    await session_service.create_session(
        app_name="design-doc-agent", user_id="test_user", session_id="test_session"
    )
    runner = Runner(
        agent=root_agent, app_name="design-doc-agent", session_service=session_service
    )
    query = "Create a design doc from this draft: The new feature will be a real-time chat component for our application. It should support multiple users in a channel and store message history."
    async for event in runner.run_async(
        user_id="test_user",
