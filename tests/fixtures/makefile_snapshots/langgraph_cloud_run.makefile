...
deploy:
	...
	gcloud beta run deploy test-langgraph \
		...
		--update-env-vars \
		"APP_URL=https://test-langgraph-$$PROJECT_NUMBER.us-west1.run.app" \
		$(if $(IAP),--iap) \
		$(if $(PORT),--port=$(PORT))
...
register-gemini-enterprise:
	...
	uvx agent-starter-pack@0.20.0 register-gemini-enterprise \
		--agent-card-url="https://test-langgraph-$$PROJECT_NUMBER.us-west1.run.app/a2a/test_langgraph/.well-known/agent-card.json" \
