...
deploy:
	...
	(gcloud compute networks describe test-langgraph-network --project $$PROJECT_ID >/dev/null 2>&1 || \
		(echo "Creating VPC network..." && \
		 gcloud compute networks create test-langgraph-network --subnet-mode=custom --project=$$PROJECT_ID)) && \
	echo "Ensuring subnet exists..." && \
	(gcloud compute networks subnets describe test-langgraph-subnet --region us-central1 --project $$PROJECT_ID >/dev/null 2>&1 || \
		(echo "Creating subnet in us-central1..." && \
		 gcloud compute networks subnets create test-langgraph-subnet --network=test-langgraph-network --region=us-central1 --range=10.0.0.0/20 --project=$$PROJECT_ID)) && \
	echo "Ensuring firewall rules exist..." && \
	(gcloud compute firewall-rules describe test-langgraph-allow-internal --project $$PROJECT_ID >/dev/null 2>&1 || \
		(echo "Creating internal firewall rule..." && \
		 gcloud compute firewall-rules create test-langgraph-allow-internal \
			--network=test-langgraph-network \
			--allow=tcp,udp,icmp \
			--source-ranges=10.0.0.0/8 \
			--project=$$PROJECT_ID)) && \
	echo "Ensuring GKE cluster exists..." && \
	(gcloud container clusters describe test-langgraph-dev --region us-central1 --project $$PROJECT_ID >/dev/null 2>&1 || \
		(echo "Creating GKE Autopilot cluster (this may take a few minutes)..." && \
		 gcloud container clusters create-auto test-langgraph-dev --region us-central1 --project $$PROJECT_ID --network=test-langgraph-network --subnetwork=test-langgraph-subnet)) && \
	...
	(gcloud artifacts repositories create test-langgraph \
	...
	kubectl create namespace test-langgraph --dry-run=client -o yaml | kubectl apply -f - && \
	...
	SA_EMAIL=test-langgraph-app@$$PROJECT_ID.iam.gserviceaccount.com && \
	(gcloud iam service-accounts describe $$SA_EMAIL --project=$$PROJECT_ID >/dev/null 2>&1 || \
		gcloud iam service-accounts create test-langgraph-app \
			--display-name="test-langgraph Agent Service Account" \
			--project=$$PROJECT_ID) && \
	...
	gcloud iam service-accounts add-iam-policy-binding $$SA_EMAIL \
		...
		--member="serviceAccount:$$PROJECT_ID.svc.id.goog[test-langgraph/test-langgraph]" \
		--project=$$PROJECT_ID --quiet --no-user-output-enabled && \
	kubectl annotate serviceaccount test-langgraph \
		iam.gke.io/gcp-service-account=$$SA_EMAIL \
		-n test-langgraph --overwrite && \
	echo "Deploying container image..." && \
	sed 's|image: PLACEHOLDER|image: '"$$IMAGE"'|' k8s/deployment.yaml | kubectl apply -f - && \
	echo "Waiting for rollout to complete..." && \
	kubectl rollout status deployment/test-langgraph -n test-langgraph && \
	echo "" && \
	...
	if [ -n "$$EXTERNAL_IP" ]; then \
		kubectl set env deployment/test-langgraph \
...
register-gemini-enterprise:
	...
	uvx agent-starter-pack@0.20.0 register-gemini-enterprise \
		--agent-card-url="http://$$EXTERNAL_IP:8080/a2a/test_langgraph/.well-known/agent-card.json" \
